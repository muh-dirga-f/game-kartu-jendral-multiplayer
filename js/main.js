...
import { createRoom, joinRoom, setReady, claimHost } from "./multiplayer/room-manager.js";
...
const state = { roomId: null, room: null, isHost: false };
...
async function init() {
  await initNetwork(window.FIREBASE_CONFIG);
}
...
function render() {
  ...
  renderHands(ui.hand, players, state.room.game?.hands || {}, me, async (cardId) => {
    if (state.isHost) await playCardAsHost(state.roomId, state.room, me, cardId);
  });
  pulse(ui.players);
}
